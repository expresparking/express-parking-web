...
export default function AboutPage() {
  return <main>
    ...
    <section className="section shell story-grid">
      ...
      <div><p><strong>Express Parking</strong> handles vehicle access, garages, lots, payment systems, monthly parking, and valet operations.</p><p><strong>Express Property Care</strong> provides local keyholding, site checks, vendor access, documentation, and emergency dispatch for owners.</p><p><strong>Velor Car Care</strong> provides waterless mobile detailing and non-toxic interior vehicle care.</p></div>
    </section>
    ...
    <section className="client-section"><div className="shell client-grid"><div><span className="eyebrow light"><span className="eyebrow-line" />Who we serve</span><h2>Connecticut properties with real operating needs.</h2></div><div className="client-types">{["Parking owners + operators", "Commercial + mixed-use properties", "Healthcare + medical campuses", "Residential communities", "Universities + education campuses", "Hospitality + event venues", "Municipal + district buyers"].map(item => <span key={item}>{item}</span>)}</div></div></section>
  </main>;
}
